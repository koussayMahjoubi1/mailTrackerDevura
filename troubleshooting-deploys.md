...
- The values of important [environment variables](configure-environment-variables)
...
Whenever your app misbehaves in any way, *always check the logs first.* Logs are available in the [Render Dashboard][dboard]:
...
- To use any tools or utilities besides those [included by default](native-runtimes#tools-and-utilities) in Render's native runtimes, make that sure you install them as part of your app's [build command](deploys#deploy-steps).
...
- *Missing [environment variables](environment-variables):* Some apps require certain environment variables to be set for them to build and start successfully.
  - Add environment variables to your app in the [Render Dashboard][dboard], or via a `render.yaml` [blueprint file](blueprint-spec).
- *Missing [Dockerfile](docker) `CMD` or `ENTRYPOINT`:* If you build and run your app from a `Dockerfile`, that file must include a `CMD` or `ENTRYPOINT` directive.
  ...
  - If you omit _both_ of these directives, your deploy might appear to hang indefinitely in the [Render Dashboard][dboard].
...
- A service or database is overwhelmed, often by too many concurrent connections or constrained resources (such as CPU or RAM).
  - In this case, warnings about resource constraints usually appear in the service's [logs](logging) and on the service's Events page in the [Render Dashboard][dboard].
